...
import { Mail, Lock, Eye, EyeOff, GraduationCap, Briefcase, ArrowRight, Zap, AlertTriangle } from 'lucide-react';
...
const Login: React.FC = () => {
  ...
  const [selectedRole, setSelectedRole] = useState<'student' | 'counsellor' | null>(null);
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/80 p-8 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { role: 'student' as const, icon: GraduationCap, label: 'Student', desc: 'Career guidance & tools' },
                { role: 'counsellor' as const, icon: Briefcase, label: 'Counsellor', desc: 'Manage & guide students' },
              ].map(({ role, icon: Icon, label, desc }) => (
                <motion.button key={role} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                  onClick={() => { setSelectedRole(role); setError(''); }}
                  className={`p-4 rounded-2xl border-2 text-left transition-all ${selectedRole === role
                    ? 'border-indigo-500 bg-indigo-50'
                    : 'border-slate-200 hover:border-indigo-200'}`}
                >
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center mb-2 ${selectedRole === role ? 'bg-indigo-500' : 'bg-slate-100'}`}>
                    <Icon className={`w-5 h-5 ${selectedRole === role ? 'text-white' : 'text-slate-500'}`} />
                  </div>
                  <p className={`font-bold text-sm ${selectedRole === role ? 'text-indigo-700' : 'text-slate-700'}`}>{label}</p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
              ...
              {loading
                ...
                : <><ArrowRight className="w-4 h-4" /> Sign In as {selectedRole ? (selectedRole === 'student' ? 'Student' : 'Counsellor') : '...'}</>}
            </motion.button>
          </form>
